...
export const Button: React.FC<ButtonProps> = ({ kind, className, borderColor, textColor, children, ...props }) => (
  <button
    className={`
                ...
                ${
                  kind === 'filter' &&
                  `flex flex-row gap-4 rounded-2xl border-2 border-${borderColor || 'neutral/40'} py-4 px-6 ${
                    textColor || 'primary/60'
                  }`
                }
                ${
                  kind === 'filterHashtagOption' &&
                  `flex flex-row gap-4 rounded-full border-2 border-${
                    borderColor || 'primary/40'
                  } bg-primary/20 py-2 px-4 ${textColor || 'primary/40'}`
                }
                ${
                  kind === 'filterStatusOption' &&
                  `flex flex-row gap-4 rounded-full border-2 border-${
                    borderColor || 'neutral/40'
                  } bg-neutral/10 py-2 px-4 ${textColor || 'neutral/40'}`
                }
